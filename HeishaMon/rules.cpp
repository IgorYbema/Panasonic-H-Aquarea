...
static int is_variable(char *text, unsigned int *pos, unsigned int size) {
  ...
  if(size == strlen_P(PSTR("ds18b20#2800000000000000")) && strncmp_P((const char *)&text[*pos], PSTR("ds18b20#"), 8) == 0) {
    ...
    if(text[*pos] == '%') {
      if(size == 4 && strnicmp(&text[(*pos)+1], "hour", 4) == 0) {
        return 5;
      }
      if(size == 5 && strnicmp(&text[(*pos)+1], "month", 5) == 0) {
        return 6;
      }
    }
    ...
    if(text[*pos] == '@') {
      ...
      for(x=0;x<nrcommands;x++) {
        ...
        if(size == len && strnicmp(&text[(*pos)+1], cmd.name, len) == 0) {
          i = len+1;
          match = 1;
          break;
        }
      ...
      if(match == 0) {
        ...
        for(x=0;x<nrtopics;x++) {
          ...
          if(size == len && strnicmp(&text[(*pos)+1], cpy, len) == 0) {
            i = len+1;
            match = 1;
            break;
          }
