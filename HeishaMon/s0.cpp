...
void s0Timer(uint8_t i, PubSubClient &mqtt_client, void (*log_message)(char*), char* mqtt_topic_base, s0SettingsStruct s0Settings[]) {
  ...
  sprintf(mqtt_topic, "%s/%s/Watthour/%d", mqtt_topic_base, mqtt_topic_s0, (i + 1));
  ...
  sprintf(log_msg, "Measured total Watthour on S0 port %d: %.2f", (i + 1),  WatthourTotal );
  log_message(log_msg);
  sprintf(valueStr, "%.2f", WatthourTotal);
  sprintf(mqtt_topic, "%s/%s/WatthourTotal/%d", mqtt_topic_base, mqtt_topic_s0, (i + 1));
  mqtt_client.publish(mqtt_topic, valueStr, MQTT_RETAIN_VALUES);
  sprintf(log_msg, "Calculated Watt on S0 port %d: %u", (i + 1), actS0Data[i].watt);
  log_message(log_msg);
  sprintf(valueStr, "%u",  actS0Data[i].watt);
  sprintf(mqtt_topic, "%s/%s/Watt/%d", mqtt_topic_base, mqtt_topic_s0, (i + 1));
  mqtt_client.publish(mqtt_topic, valueStr, MQTT_RETAIN_VALUES);
}
